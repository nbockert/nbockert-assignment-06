...
def generate_plots(N, mu, sigma2, S):
    ...
    X = np.random.rand(N, 1)  # N random values between 0 and 1
    Y = mu + np.random.normal(0, np.sqrt(sigma2), N).reshape(-1, 1) 
    ...
    for _ in range(S):
        ...
        Y_sim = mu + np.random.normal(0, np.sqrt(sigma2), N).reshape(-1, 1)  # Replace with code to generate Y values

        # TODO: Fit a linear regression model to X_sim and Y_sim
        sim_model = LinearRegression().fit(X_sim, Y_sim)  # Initialize model and fit model
